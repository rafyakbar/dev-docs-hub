...
You may add any [form components](../forms) to the `form()` method, and create the team inside the `handleRegistration()` method. Now, we need to tell Filament to use this page. We can do this in the [configuration](../panel-configuration):
...
You may add any [form components](../forms) to the `form()` method. They will get saved directly to the tenant model. Now, we need to tell Filament to use this page. We can do this in the [configuration](../panel-configuration):
...
The tenant-switching menu is featured in the admin layout. It’s fully customizable. Each menu item is represented by an [action](../actions), and can be customized in the same way. To register new items, you can pass the actions to the `tenantMenuItems()` method of the [configuration](../panel-configuration):
...
[Edit this page on GitHub](https://github.com/filamentphp/filament/edit/5.x/docs/07-users/03-tenancy.md)
