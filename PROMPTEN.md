...
I previously succeeded in crawling the Livewire v4 documentation using Python in Jupyter Lab, as shown in the example above. Now I want to crawl the FluxUI v2 documentation starting from the following page:

[https://fluxui.dev/docs/installation](https://fluxui.dev/docs/installation)

Please help me create a script with the following requirements:

1. I am using Jupyter Lab, so the code must be directly executable as a Python cell.

2. The structure must strictly follow the same pattern I used before, namely:
...
3. Every section of the code must include explanatory comments in two languages:

   * EN for English explanations
   * ID for Indonesian explanations

4. The documentation menu is usually located inside the following element:
...
5. An example of a menu item is:
...
6. The main documentation content is usually located inside the following element:
...
7. When converting HTML to Markdown:

   * First check whether there are any image-related tags.
   * If tags such as img, svg, picture, or source are found, remove them before conversion.
   * This is to prevent the final Markdown file from becoming unnecessarily large.

8. The final result must include:

   * All documentation pages saved as separate HTML files.
   * All HTML files combined into individual Markdown files and a single final Markdown file.
   * Files ordered using numeric prefixes such as 0001, 0002, and so on.

Please generate a complete and detailed script that follows all the specifications above.